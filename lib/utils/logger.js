...
function message(err){
  switch (err.type) {
    case 'directory_required':
      return 'You must initialize a component with a directory.';
      break;

    case 'json_not_found':
      return 'There is no component.json in this directory.';
      break;

    case 'remote_not_found':
      return 'Could not find "' + err.repository + '".';
      break;

    case 'version_not_found':
      return 'Could not find a version for "' + err.repository + '".';
      break;

    case 'version_mismatch':
      var brk = '\n                 ';
      var msg = 'There was a version mismatch for "' + err.repository + '":';
      msg += brk;
      msg += brk;
      return msg + Object.keys(err.versions).map(function(name){
