 ...
 * the given `pkg` name, `version` and `dest` dir.
 *
 * @param {String} pkg
...
function Installer(pkg, version, dest) {
  debug('installing %s@%s to %s', pkg, version, dest);
...
Installer.prototype.getDependencies = function(deps, fn){
  ...
  Object.keys(deps).forEach(function(name){
    ...
    batch.push(function(done){
      var pkg = new Installer(name, version, self.dest);
      self.emit('dep', pkg);
      pkg.on('end', done);
      pkg.install();
    });
